...
void TFT::fillScreen_random(INT16U XL,INT16U XR,INT16U YU,INT16U YD)
{
    unsigned long  XY = 0;
...
void TFT::fillRectangle_t(INT16U poX, INT16U poY, INT16U length, INT16U width, INT16U color)
{
    for(int x=poX; x<(poX+length); x++)
    for(int y=poY; y<(poY+width); y++)
    {
        int x1 = 239-y;
        int y1 = x;
        setPixel(x1, y1, color == BLACK ? BLACK : random(0xffff));
    }
}
...
void TFT::drawChar( INT8U ascii, INT16U poX, INT16U poY,INT16U size, INT16U fgcolor)
{
    ...
    for (int i =0; i<FONT_X; i++){
        ...
        for(INT8U f=0;f<8;f++)
        {
            if((temp>>f)&0x01)
            {
                fillRectangle_t(poX+i*size, poY+f*size, size, size, fgcolor);
            }
            else
                fillRectangle_t(poX+i*size, poY+f*size, size, size, BLACK);
